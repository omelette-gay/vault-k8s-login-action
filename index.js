...
async function obtainK8sCredentials({ httpClient, vaultServer, k8sMountpoint, k8sRole, k8sNamespace, }) {
    const response = await httpClient.post(`${vaultServer}/v1/${k8sMountpoint}/creds/${k8sRole}`, JSON.stringify({ kubernetes_namespace: k8sNamespace }));
    if (response.message.statusCode !== http_client_1.HttpCodes.OK) {
...
(0, core_1.getIDToken)()
    ...
    .then((vaultToken) => {
    const authHandler = new auth_1.BearerCredentialHandler(vaultToken);
