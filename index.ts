...
async function authenticateToVault({
  ...
  if (response.message.statusCode !== HttpCodes.OK) {
    throw new VaultError(
      `Unexpected response from Vault when authenticating: ${response.message.statusCode}`,
    );
  }
...
async function obtainK8sCredentials({
  ...
  if (response.message.statusCode !== HttpCodes.OK) {
    throw new VaultError(
      `Unexpected response from Vault when obtaining k8s credentials: ${response.message.statusCode}`,
    );
  }
